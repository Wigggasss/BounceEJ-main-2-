...
async function submitLeaderboardScore(nameOverride = leaderboardNameInput.value, isAutoSubmit = false) {
  ...
  const saveResult = await saveLeaderboardScore(playerName, true, previousLockedName);

  if (saveResult.error) {
    submitScoreButton.disabled = false;
    leaderboardNameInput.disabled = Boolean(getLockedLeaderboardName());
    setLeaderboardSubmitMessage(saveResult.taken ? "That name is already taken." : "Could not save score.", "error");
    return;
  }
...
async function saveLeaderboardScore(playerName, retryDuplicate = true, previousLockedName = "") {
  const row = {
    ...
    mode_label: game.modeLabel,
    character_id: saveState.equippedCharacter,
    xp_earned: game.finalXpEarned || 0
  };
  ...
  const playerNameKey = getLeaderboardNameKey(playerName);
  ...
  if (insertError && insertError.code === "23505" && retryDuplicate) {
    return saveLeaderboardScore(playerName, false, previousLockedName);
  }
    ...
    if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") {
      ...
      if (game && game.onlineDuel && multiplayer.status === "playing" && !multiplayer.result) {
        finishMultiplayerMatch({
          winnerId: getOtherMultiplayerPlayerId(multiplayer.playerId),
          loserId: multiplayer.playerId,
          reason: "disconnect",
